...
const HighestWrapper = ({ refreshKey }: { refreshKey: any }) => {
  const [curr, setCurr] = useState("USD");
  const { highestRate, error, loading } = useHighestExchange(curr);

  // Use the key to trigger a refresh of the useHighestExchange hook
  useEffect(() => {
    // This will rerun whenever the key changes, thus re-fetching the data
  }, [refreshKey]);
